...
public class Demo3 {
	...
	private static void zeigeFenster(String title) {
        ...
        linkerBalken.setForeground(Color.orange);
	...
	public static void main(String[] args) {
		...
		for (int i=1; i<=100; i++) {
			...
			// asynchroner Methodenaufruf mit Aufruferblockierung
			if (i == 50) {
				Thread thread = new Thread( () -> methode() );
				thread.start();
				try {
					// aktuellen Thread blockieren, damit es wie ein synchroner Aufruf aussieht
					thread.join();
				}
				catch (Exception ignored) {}
			}
